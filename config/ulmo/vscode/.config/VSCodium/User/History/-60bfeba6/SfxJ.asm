...
dcrp_mcr macro
    ...
    lea ebx, a1_netyaga
    xor_l:
        mov al, [esi] ; comment in dcrp_mcr
        mov cl, [edi] ;; hidden_comment_in_dcrp_mcr
        xor al, cl
        mov [ebx], al
        inc esi
        inc edi
        ...
        mov byte ptr [ebx], 0
endm
lst_inf_ macro x, y  ; thng_to_show_here
    invoke MessageBoxW, 0, addr x, addr y, 0 ;; hidden_thng_show_&ere
endm
cmpr__ macro xx, yy
    LOCAL locai ;local
    locai:
        invoke lstrcmp, addr xx, addr yy ;;local-label-for-local-in-local
endm
.code
x2 proc h1:dword, u1:dword, w1:dword, l1:dword
    ...
    .endif
        .if w1 == 8
                ...
                .if EAX == 0 ; invoke MessageBoxW, 0, offset b2_netyaga, offset b1_netyaga, 0
