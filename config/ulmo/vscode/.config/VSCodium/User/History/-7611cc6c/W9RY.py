    ...
    def run(self):
        for file_i, file_path in enumerate(self.files):
            ...
            if file_path.suffix == '.pdf':
                ...
                shutil.copy(file_path, output_tmp_path)
            else:
                self.processed.emit(file_i, False)
                return
            ...
            for element in soup.descendants:
                if isinstance(element, NavigableString):
                    old_text = new_text = element.get_text(
                        strip=True, separator=' '
                    )
                    print(self.patterns_to_remove)
                    for pattern in self.patterns_to_remove:
                        new_text = pattern.sub(
                            f'$ВИДАЛЕНО${self.rm_counter}', old_text
                        )
                        # print(new_text)
                    print(new_text)
                ...
                for p in page.find_all('p'):
                    ...
                    for top_to_remove in tops_to_remove:
                        if (
                            ...
                            p.clear()
                            p.append(f'$ВИДАЛЕНО${self.rm_counter}')
                            self.rm_counter += 1
...
class MainWindow(QMainWindow):
    ...
    def open_file(self):
        ...
        choice, _ = QFileDialog.getOpenFileName(
            self,
            'Обрати файл',
            filter='PDF or HTML files (*.pdf *.html);;PDF files (*.pdf);;HTML files (*.html)',
