...
class FileProcessor(QObject):
    processed = pyqtSignal(int, bool, Path)  # filename, content
        ...
        for file_i, path in enumerate(self.files):
            ...
            convertion_process = subprocess.run(
                [
                    ...
                    '-noframes',
                    path,
                    TMP_RAW_PATH / path.with_suffix('.html').name,
                ],
                capture_output=True,
                text=True,
            )
            ...
            for page in soup.body.find_all('div'):
                ...
                for p in page.find_all('p'):
                    ...
                    if p.get_text().strip(': ') in self.fields_to_remove:
            ...
            self.processed.emit(file_i, True, output_tmp_path)
...
class MainWindow(QMainWindow):
    ...
    def open_file(self):
        ...
        choice, _ = QFileDialog.getOpenFileName(
            self, 'Обрати файл', filter='PDF files (*.pdf)'
        )
    ...
    def open_directory(self):
        ...
        dir_choice = QFileDialog.getExistingDirectory(self, 'Обрати теку')
        if not dir_choice:
            return
    ...
    def on_file_processed(
        self, file_index: int, status: bool, file_path: Path | None = None
    ):
    ...
    def save_selected_file(self):
        ...
        if not item:
            return
        filename = item.text() + '_оброблено.html'
        save_path, _ = QFileDialog.getSaveFileName(
            self, 'Зберегти файл', filename
        )
        if save_path:
    ...
    def save_all_files(self):
        folder = QFileDialog.getExistingDirectory(
            self, 'Обрати теку для збереження всіх файлів'
        )
        if not folder:
            return
        ...
        for file in self.processed_files:
            tmp_path = file.with_name(file.stem + '_оброблено.html')
    ...
    def save_archive(self):
        first_item = self.processed_files[0]
        if first_item is None:
            return
        archive_name = first_item.with_name(first_item.stem + '_та_інші.zip')
        archive_path, _ = QFileDialog.getSaveFileName(
            self, 'Зберегти архів', str(archive_name)
        )
        if not archive_path:
            return
        ...
        for file in self.processed_files:
            file = file.with_name(file.stem + '_оброблено.html')
