...
SECTION_SYNONYMS = {
    "Нормативна грошова оцінка": "Відомості про нормативно грошову оцінку ділянки"
}
...
def extract_main_attributes(table: Tag) -> Dict:
    ...
    for row in table.find_all("tr"):
        ...
        if len(row) == 1:
            header = row[0]
            if header in SECTION_SYNONYMS:
                header = SECTION_SYNONYMS[header]
            if header == "":
    ...
    for key in main_attributes:
        main_attributes[key] = to_tuple(main_attributes[key])
