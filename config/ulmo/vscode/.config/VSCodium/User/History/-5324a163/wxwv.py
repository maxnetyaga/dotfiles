...
from .html_parser.extractors import extract_land_plot

# table = BeautifulSoup(input_path.read_text(
#         encoding="utf-8"), "html.parser").find("table")
...
def sort(origin, new):
    ...
    (RESULT_DIR / "modified").mkdir(parents=True)
    (RESULT_DIR / "unmodified").mkdir(parents=True)
    ...
    archived = origin_keys - set(new_index.keys())
    new = set(new_index.keys()) - origin_keys
    rest = 
