...
def sort(origin, new):
    ...
    origin_keys = set(origin_index.keys())
    ...
    for (old, new) in zip((origin_index[x] for x in rest),
                          (new_index[x] for x in rest)):
        old_table = BeautifulSoup(old.read_text(encoding="utf-8"), "lxml").find("table")
        new_table = BeautifulSoup(new.read_text(encoding="utf-8"), "lxml").find("table")

        if not isinstance(old_table, Tag):
            logger.warning(f"{Fore.RED}No table in old table {old}!{Fore.RESET}")
            return
        if not isinstance(new_table, Tag):
            logger.warning(f"{Fore.RED}No table in new table {new}!{Fore.RESET}")
            return

        if extract(old_table) == extract(new_table):
