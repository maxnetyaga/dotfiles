...
def dict_of_dicts_diff(old, new):
    diff = {
        'added': {},
        'removed': {},
        'changed': {}
    ...
    for key in old_keys & new_keys:
        ...
        for inner_key in set(old_inner) | set(new_inner):
            old_val = old_inner.get(inner_key)
            new_val = new_inner.get(inner_key)
            if old_val != new_val:
                changes[inner_key] = {'old': old_val, 'new': new_val}

        if changes:
            diff['changed'][key] = changes
...
def sort(origin, new):
    ...
    for num in rest:
        ...
        if old_extracted == new_extracted:
            ...
            print("NOO")
            print(dict_of_dicts_diff(old_extracted, new_extracted))
