...
CalculateAndDisplay proc hWnd:HWND
    ...
    local numerator:REAL10      ; Using REAL10 (80-bit) for maximum precision
    ...
    .while esi < 6
        ...
        ; Clear FPU stack before calculations (safety measure)
        finit
        ...
        fstp st(0)                           ; Clear stack after test
        
        ; IMPROVED CALCULATION OF NUMERATOR: (4*lg(c) - b/2 + 23)
        ; Maximize precision by keeping values in FPU registers as long as possible
        
        ; Calculate log10(c) with maximum precision
        fld qword ptr [netyaga_c_vals + esi]  ; Load c
        fld1                                 ; Load 1.0
        fyl2x                                ; Compute ln(c)
        fldl2t                               ; Load log2(10)
        fdivp st(1), st(0)                   ; Convert to log10(c)
        
        ; Multiply by 4
        fld1                                 ; Load 1.0
        fld1                                 ; Load 1.0
        faddp st(1), st(0)                   ; 2.0
        fld st(0)                            ; Duplicate 2.0
        faddp st(1), st(0)                   ; 4.0
        ...
        fsubp st(1), st(0)                   ; 4*log10(c) - b/2
        
        ; Add 23 (using a direct constant load)
        fild dword ptr [fp_twenty_three]        ; Load integer 23
        faddp st(1), st(0)                   ; 4*log10(c) - b/2 + 23
        
        fstp tbyte ptr [numerator]           ; Store as REAL10 (dt)
        
        ; IMPROVED CALCULATION OF DENOMINATOR: d - a + 1
        fld qword ptr [netyaga_d_vals + esi] ; Load d
        fld qword ptr [netyaga_a_vals + esi] ; Load a
        fsubp st(1), st(0)                   ; d - a
        fld1                                 ; Load 1.0
        faddp st(1), st(0)                   ; d - a + 1
        fstp tbyte ptr [denominator]         ; Store as REAL10
        
        ; Check for division by zero with higher precision
        fld tbyte ptr [denominator]
        fldz                                 ; Load exact 0.0
        fcomip st(0), st(1)                  ; Compare 0 with denominator and pop
        je div_zero_error                    ; Jump if denominator = 0
        
        ; Calculate division with maximum precision
        fld tbyte ptr [numerator]            ; Load numerator
        fdiv st(0), st(1)                    ; numerator / denominator
        fstp tbyte ptr [result]              ; Store full precision result
        fstp st(0)                           ; Clear denominator from stack
        
        ; Convert to double for display (maintaining as much precision as possible)
        fld tbyte ptr [result]               ; Load extended precision result
        fstp qword ptr [final_result_val]    ; Convert to REAL8 for display
        
        ; Convert final result to string with more precision digits
    ...
    div_zero_error:
        fstp st(0)  ; Clear FPU stack after comparison
        invoke AppendText, addr division_by_zero
        
    next_iteration:
        invoke AppendText, addr separator
        finit        ; Clear entire FPU stack between iterations
