...
CalculateAndDisplay proc hWnd:HWND
    ...
    .while esi < 6
        ...
        fmul tbyte ptr [fp_four]                   ; 4*log10(c)

        fld tbyte ptr [netyaga_b_vals + esi]    ; b
        fdiv tbyte ptr [fp_two]                    ; b/2

        fsubp st(1), st(0)                         ; 4*lg(c) - b/2

        fadd tbyte ptr [fp_twenty_three]           ; +23
        fstp numerator                             ; store numerator (REAL10)

        ; Calculate denominator: d - a + 1
        fld tbyte ptr [netyaga_d_vals + esi]
        fsub tbyte ptr [netyaga_a_vals + esi]
        fld tbyte ptr [fp_one]
        fadd
        fstp denominator
