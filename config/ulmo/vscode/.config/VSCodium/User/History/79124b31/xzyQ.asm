...
FloatToStr proc uses esi edi ebx value:QWORD, buffer:DWORD
    local format_str[16]:byte
    
    ; We'll use sprintf to convert floating point to string
    invoke lstrcpy, addr format_str, addr fp_format
    
    ; Call sprintf to convert the floating point value to string
    invoke crt_sprintf, buffer, addr format_str, 
           dword ptr value, dword ptr value[4]
...
CalculateAndDisplay proc hWnd:HWND
    ...
    .while esi < 6
        ...
        invoke FloatToStr, final_result_val, addr str_intermediate
        invoke wsprintf, addr temp_buf, addr intermediate_result, addr str_intermediate
        invoke AppendText, addr temp_buf
