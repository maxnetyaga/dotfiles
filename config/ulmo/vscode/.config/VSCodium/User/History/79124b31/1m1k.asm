...
.data
    ...
    ; Changed format strings to use %g instead of %.3f to handle floating point better
    values db "Values: a = %g, b = %g, c = %g, d = %g",0
    expression db "Expression: (4*lg(%g) - %g/2 + 23) / (%g - %g + 1)",0
    intermediate_result db "Intermediate result: %g",0
    final_result db "Final result: %g",0
